...
A=histogram ( Y_samples(:,1), [min(Y_samples(:,1)):5:max(Y_samples(:,1))], 'Normalization','probability' );
...
%writematrix(Y_samples(:,1), 'Y_samples_data_1e4.txt');
% writematrix(A.Values, 'Probability_1e4.txt');
aa = A.Values;
save Probability_1e4.txt -ascii aa

pre = load('final_px_scho.txt');